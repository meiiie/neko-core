...
1. **Chốt mã từ NGUỒN tốt nhất có:**
   - **Có ẢNH sản phẩm** → đọc bao bì bằng **model vision** (gpt-oss không có thị giác). **DÙNG `NEKO_MODEL=nvidia/llama-3.1-nemotron-nano-vl-8b-v1`** (NVIDIA NIM, tinh chỉnh cho tài liệu/OCR) — **ĐO THỰC (3 lần nhất quán): đọc ĐÚNG `SanDisk Cruzer Blade/CZ50 USB 2.0 16GB`, kể cả mã `CZ50`**, ngang ChatGPT, **dùng key NVIDIA sẵn có**. (`nvidia/nemotron-nano-12b-v2-vl` cũng tốt. **TRÁNH** `llama-3.2-vision`: 11B đọc nhầm dung lượng, 90B bịa sản phẩm.) Hoặc **screenshot + vision** (computer-use).

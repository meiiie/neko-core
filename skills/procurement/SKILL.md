...
Canonical flow: `U9 + RTX 5070 + 16 inch` → broad INDEX thấy `83KY001VVN` → chạy `neko procurement source-plan 83KY001VVN --category laptop` → query FPT và An Khang riêng → verify trang gốc → code tính max. Cấu hình giống nhau chỉ là **candidate relation**; SKU khớp mới là **exact identity**.
...
- **Có ảnh:** đọc dòng/dung lượng/mã trên bao bì bằng vision (`neko run --profile nvidia --image <ảnh> "Sản phẩm gì?"`). Resize/crop ảnh; mã đầy đủ thường ở mặt sau. Vision chưa đọc được mã thì giữ là candidate, không tự bịa SKU.
...
- **Browser MCP cho sàn động (Shopee/Tiki/Lazada/TikTok)**: các sàn này render giá bằng JS → `web_fetch` tĩnh nhận vỏ rỗng. Nếu có tool `mcp__playwright__browser_*` (đã cấu hình) thì: `mcp__playwright__browser_navigate` mở trang → `mcp__playwright__browser_snapshot` đọc DOM ĐÃ RENDER (thấy giá) → rồi bóc như thường. Không có thì ghi "cần người mở link xác minh".
  - *Bật:* chạy `neko setup browser` để dùng Chrome profile bền vững (đăng nhập một lần); `neko setup browser attach` để dùng tab Chrome đã đăng nhập; `isolated` chỉ dành cho kiểm thử tạm thời.

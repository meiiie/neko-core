...
function factory(overrides: { capability?: boolean; itemStatus?: string } = {}): { factory: ImageClientFactory; requests: string[] } {
  ...
  const make: ImageClientFactory = (handlers: CodexAppServerHandlers) => ({
    ...
    request: async (method: string) => {
      ...
      if (method === "thread/start") return { thread: { id: "t-img" } };
...
test("generateImage: capability checked first, PNG lands at the requested project path", async () => {
  ...
  try {
    const { factory: make, requests } = factory();

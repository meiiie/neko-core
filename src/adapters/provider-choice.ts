...
export function providerChoices(cfg: NekoConfig, authOnly = false): Choice[] {
  ...
  for (const name of Object.keys(cfg.profiles).sort()) {
    ...
    choices.push({
      id: family,
      label: family === "openai" ? "OpenAI" : family,
...
export function authChoices(cfg: NekoConfig, family: string, availability: AuthAvailability): Choice[] {
  return Object.entries(cfg.profiles)
    ...
    .sort(([, a], [, b]) => (a.auth === "chatgpt_oauth" ? -1 : b.auth === "chatgpt_oauth" ? 1 : 0))
    .map(([name, profile]) => {
      const ready = profile.auth === "none" || (profile.auth === "chatgpt_oauth" ? availability.chatgpt : availability.apiProfiles.has(name));
      const billing = profile.auth === "none" ? "no sign-in required" : profile.auth === "chatgpt_oauth" ? "subscription, no API billing" : "pay-as-you-go API";
      return {
        id: name,
...
export function profileDisplayName(cfg: NekoConfig): string {
  ...
  const family = profile?.family === "openai" ? "OpenAI" : (profile?.family || cfg.profile);

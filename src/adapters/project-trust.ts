...
function validManifest(files: any): files is Record<string, string> {
  ...
  for (const [name, digest] of Object.entries(files)) {
    // SAFETY: key membership in PROJECT_CONTROL_FILES is checked just above.
...
function validateRecord(id: string, raw: any): TrustRecord {
  ...
  // SAFETY: store bytes were just hash- and schema-verified by the trust reader.
...
function assertRootIdentity(root: string, expectedIdentity: string): Stats {
  // SAFETY: stat result of a path this code just resolved and checked.

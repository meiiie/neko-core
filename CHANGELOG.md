...
- **`/fullscreen` — a scrollable alt-screen mode (scroll up while it streams; jump back to the latest)** —
  ...
  is still streaming** without being yanked to the bottom; **Home/End** jump to the top/bottom, a `↓ N rows
  below` pill shows when you're scrolled up, and **End / PageDown / Esc** (or sending a message) jumps back to
  the latest. Stock Ink can't clip a viewport

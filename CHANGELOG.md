...
- **Completed tool activity collapses to one useful outcome line.** Successful calls use compact past-tense
  ...
  folded away. MCP protocol-level `isError`, interrupted commands, and missing-skill outcomes remain
  expanded, and search/glob summaries name the requested pattern rather than their base directory.

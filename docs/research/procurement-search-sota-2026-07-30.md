...
4. The procurement skill puts the identifier cascade and completion contract before detailed tactics,
   invokes the planner through standalone `neko procurement source-plan` or source-checkout
   `bun bin/neko.ts procurement source-plan`, and allows all available exact matches when fewer than three candidates exist.
